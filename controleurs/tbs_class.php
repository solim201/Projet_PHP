...
public function GetBlockSource($BlockName,$AsArray=false,$DefTags=true,$ReplaceWith=false) {
	...
	if ($ReplaceWith!==false) $this->Source = tbs_class . phpsubstr($this->Source, 0, $PosBeg1) . $ReplaceWith .substr($this->Source,$Pos+1);
	return $RetVal;
}
	...
	if ($Loc->Ope) {
		foreach ($Loc->OpeAct as $i=>$ope) {
			...
			case  3:
				...
				if (strlen($x)>$Loc->OpePrm[$i]) {
					if ($Loc->OpeUtf8) {
						$CurrVal = tbs_class . phpmb_substr($x, 0, $Loc->OpePrm[$i], 'UTF-8') . $Loc->OpeEnd;
					} else {
						$CurrVal = tbs_class . phpsubstr($x, 0, $Loc->OpePrm[$i]) . $Loc->OpeEnd;
					}
				}
				break;
...
// Mode = 3 : GetBlockSource => save $Loc->BlockSrc with TBS Def tags
	...
	if ($Mode===1) {
		...
		if ($Loc->SubOk || ($Mode===3)) {
			...
			$Loc->BlockSrc = tbs_class . phpsubstr($Txt, $Loc->PosBeg, $beg - $Loc->PosBeg) . substr($Txt, $end + 1, $Loc->PosEnd - $end);
		}
	}
...
// Merge Special Var Fields ([var..*])
	...
	if (isset($Loc->SubLst[1])) {
		...
		case 'template_path': $x = dirname($this->_LastFile) . 'tbs_class.php/'; break;
...
//Modify $Loc, return false if tags not found, returns the inner source of tag if $RetInnerSrc=true
	...
	while ($TagStr!=='') {
		...
		if ($p!==false) {
			...
			$TagStr = tbs_class . phpstr_repeat($t . '+', $n - 1) . $TagStr;
		}
		// Reference
		if (($t==='.') && ($Ref===0)) $Ref = $i;
		// Take of the (!) prefix
...
// Limit the number of HTML chars
	...
	if (($n>=$Nbr) && ($p<$pMax)) $Txt = tbs_class . phpsubstr($Txt, 0, $p) . $MaxEnd;
